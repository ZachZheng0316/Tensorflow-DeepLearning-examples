...
TRAING_STEPS = 10000        # 训练的轮数
BATCH_SIZE = 32             # batch的大小
TRAINING_EXAMPLES = 10000   # 训练数据的个数
TESTING_EXAMPLES = 1000     # 测试数据的格式
...
def generate_data(seq):
    ...
    # 返回X和y
    return np.array(X, dtype=tf.float32), np.array(y, dtype=tf.float32)
...
def lstm_model(X, y):
    ...
    # 使用多层的LSTM结构
    lstm_cell = tf.nn.rnn_cell.BasicLSTMCell(HIDDEN_SIZE)           # 建立LSTM结构
    cell = tf.nn.rnn_cell.MultiRNNCell([lstm_cell] * NUM_LAYERS)    # 建立多层RNN模型
    x_ = tf.unpack(X, axis = 1)
    
    # 使用Tensorflow接口将多层的LSTM结构连接成RNN网络并计算其前向传播结果
    output, _ = tf.nn.rnn(cell, x_, dtype=tf.float32)
    
    # 在本问题中，只关注最后一个时刻的输出结果，
    # 该结果为下一个时刻的预测值
    output = output[-1]
...
test_X, test_y = getatgenerate_data(np.sin(np.linspace(test_start, test_end, TESTING_EXAMPLES, dtype=np.float32)))
